...
fn install_linux_autostart(exe_path: &str) -> Result<(), String> {
    ...
    // Fix 2: Add current user to `input` group so rdev (X11 XRecord) can monitor
    // keystrokes/mouse without root. Requires re-login to take effect.
    let current_user = env::var("USER").unwrap_or_else(|_| {
    ...
    if !current_user.is_empty() {
        let add_group = Command::new("sudo")
            .args(["usermod", "-aG", "input", &current_user])
            .output();
        match add_group {
            Ok(out) if out.status.success() => {
                info!("Added '{}' to input group (re-login required)", current_user);
                println!("  [input group] Added '{}' to input group.", current_user);
                println!("  NOTE: Log out and log back in for keystroke monitoring to work.");
            }
            Ok(out) => {
                warn!(
                    "usermod input group failed (non-fatal): {}",
                    String::from_utf8_lossy(&out.stderr)
                );
            }
            Err(e) => warn!("usermod not available (non-fatal): {}", e),
        }
    }

    // Fix 3: Detect active X11 DISPLAY; default :0 for headless/SSH sessions.
    // scrap and rdev both require $DISPLAY — without it they crash at startup.
...
Type=Application
Name=DawellService
Exec=env DISPLAY={} {} --run
Hidden=false
NoDisplay=true
X-GNOME-Autostart-enabled=true
"#,
        display, install_path
    );
...
fn install_macos_loginitem(exe_path: &str) -> Result<(), String> {
    ...
    // Fix 1: Remove macOS Gatekeeper quarantine flag (set on downloaded binaries)
    // Without this, macOS shows "cannot be opened because the developer cannot be verified"
    let _ = Command::new("xattr").args(["-cr", &install_path]).output();

    // Ad-hoc code sign so macOS allows execution without Developer ID
    let codesign_result = Command::new("codesign")
        .args(["--force", "--deep", "--sign", "-", &install_path])
        .output();
    if let Ok(out) = &codesign_result {
